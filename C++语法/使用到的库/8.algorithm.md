...
std::lower_bound 的工作原理:
 ...
 查找方向：
   在一个已排序的数组或区间 [beg, end) 中查找目标值。
   在每次迭代中，找到当前范围的中间元素。

   将中间元素与目标值进行比较。
    如果中间元素小于目标值，目标值只能在右半部分。因此，查找方向是向右，也就是舍弃左半部分。
    如果中间元素大于目标值，目标值只能在左半部分。因此，查找方向是向左，也就是舍弃右半部分

  查找条件：在比较时，如果当前元素 < 目标值，则说明目标值应该在右边，因此查找方向是右半部分。
  在 lower_bound 中，查找的方向完全取决于比较函数的结果，即判断 a < b 是否成立。
